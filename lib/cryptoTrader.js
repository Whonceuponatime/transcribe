...
function evaluateDipSignals(sig, score) {
  return [
    ...
    { reason: 'DIP_HIGH_SCORE',       cooldown: 6, met: score >= 3,                                    val: String(score),                                          thresh: 'score ≥ 3'      },
...
async function runDipBuys(supabase, config, priceMap, analysis, availableKrw, bear, {
  ...
  for (const coin of coins) {
    ...
    // Relaxed RSI gate scales UP with how over-cashed we are.
    // At 88% KRW we're nearly fully liquidated — accept RSI < 73 (just below sell threshold).
    // Formula: base 57 + 2 pts for every 10% KRW above 40%, capped at 73.
    const relaxedRsiThresh = overCashed
      ? Math.min(57 + Math.floor((krwRatio - 40) / 10) * 2, 73)
      : 0;
    ...
    if (score < 2 && !hasOversoldSignal && !hasPullbackSignal && !hasRelaxedSignal && !hasHighScore) {
      console.log(`    ✗ SKIP — score ${score} < 2, RSI ${sig.rsi?.toFixed(1)} ≥ relaxed thresh ${relaxedRsiThresh}, no oversold/pullback`);
      continue;
    }
    if (hasHighScore && score >= 3)      console.log(`    ✓ High score (${score}) — buying regardless of RSI`);
    else if (hasPullbackSignal)          console.log(`    ✓ RSI pullback ${sig.rsi?.toFixed(1)} < 50 — swing re-entry`);
    else if (hasRelaxedSignal)           console.log(`    ✓ Over-cashed (${krwRatio?.toFixed(0)}% KRW, thresh RSI<${relaxedRsiThresh}) — re-entry RSI ${sig.rsi?.toFixed(1)}`);
