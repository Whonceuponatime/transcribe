        ...
        if (order.side === 'sell' && order.position_id && executedVol > 0) {
          ...
          if (pos) {
            ...
            for (const trade of trades) {
              try {
                await supabase.from('v2_fills').insert({
                  order_id:     order.id,
                ...
                console.error(`[recon] resolveStuckOrders: v2_fills insert failed for ${order.asset}:`, fillErr.message);
              }
            }
            ...
            if (!trades.length && executedVol > 0) {
              try {
                ...
                console.error(`[recon] resolveStuckOrders: synthetic fill insert failed for ${order.asset}:`, fillErr.message);
              }
            }
