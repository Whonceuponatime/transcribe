  ...
  try {
    ...
    if (!opts.dipBuyOnly) {
      ...
      for (const position of openPositions) {
        ...
        // Sell cooldown — prevent rapid re-sells on the same asset
        // Bypass for protective exits (regime_break, time_stop)
        const isProtectiveExit = ['regime_break', 'time_stop'].includes(exit.trim);
        ...
        if (!isProtectiveExit && (Date.now() - lastSell) < sellCooldownMs) {
          ...
          if (cycleDecisions[coin]) {
            cycleDecisions[coin].final_action = 'NO_ACTION';
            cycleDecisions[coin].final_reason = `sell_blocked:cooldown_${waitMin}min_remaining`;
          }
          continue;
        }
