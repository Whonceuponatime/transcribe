...
async function fetchHistory(apiKey) {
  ...
  const [usdkrw, broad, vix, nasdaq, dgs10, dgs2, oil, gold, sp500, fedFunds, cpi, bokRate, kospiProxy, yuanUsd, koreaTrade] = await Promise.all([
    ...
    fetchFredSeries('DCOILWTICO', apiKey, startStr),
    fetchFredSeries('GOLDAMGBD228NLBM', apiKey, startStr),
    fetchFredSeries('SP500', apiKey, startStr),
    fetchFredSeries('FEDFUNDS', apiKey, startStr),
    fetchFredSeries('CPIAUCSL', apiKey, startStr),
    fetchFredSeries('INTDSRKRM193N', apiKey, startStr),
    fetchFredSeries('SPASTT01KRM661N', apiKey, startStr),
    fetchFredSeries('DEXCHUS', apiKey, startStr),
    fetchFredSeries('XTNTVA01KRQ667S', apiKey, startStr),
  ]);
