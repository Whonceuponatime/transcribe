 ...
 * Uses primary and alternate regex to catch more cable ID formats.
 */
function findCableOccurrences(pages) {
  const occurrences = [];
  const seen = new Set(); // pageNum:pos:cableId to avoid duplicates from multiple regexes

  function addMatch(page, pageText, lines, cableIdRaw, pos) {
    const cableId = cableIdRaw.toUpperCase().replace(/\./g, '-');
    const key = `${page.pageNum}:${pos}:${cableId}`;
