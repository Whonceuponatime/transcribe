...
const PROFIT_LEVELS = [
  ...
  { gainThreshold: 80, label: '80pct', sellPct: 25, cooldownHours: 72 },
];
  ...
  for (const coin of coins) {
    ...
    if (config.signal_sell_enabled && sig && !sig.error) {
      ...
      const sellConditions = [
        sig.rsi != null && sig.rsi > 75,       // updated to match new threshold
        sig.bb?.pctB > 1.0,
        sig.macd?.bearishCross,
        sig.stochRsi != null && sig.stochRsi > 85,
        sig.vwapDev != null && sig.vwapDev > 3,
        sig.williamsR != null && sig.williamsR > -5, // updated to match new threshold
      ...
      // 2. RSI overbought — raised thresholds to reduce premature exits
      if (sig.rsi != null && sig.rsi > 85 && atProfit) {
        const done = await isRecentlySold(supabase, coin, 'rsi_ob_strong', 4);
      ...
      } else if (sig.rsi != null && sig.rsi > 75 && atProfit) {
      ...
      // 4. MACD bearish cross — only exit if we're net-profitable after fees
      if (sig.macd?.bearishCross && atProfit) {
      ...
      // 7. Williams %R overbought — raised from -15 to -5; -15 fired too easily in uptrends
      if (sig.williamsR != null && sig.williamsR > -5 && atProfit) {
        const done = await isRecentlySold(supabase, coin, 'williams_ob', 4);
...
async function runDca(supabase, config, signalScore, fearGreed, priceMap, bear, accounts, analysis = {}) {
  ...
  for (const coin of coins) {
    ...
    try {
      ...
      results.push({ coin, ok: true, krwAmount, feeKrw, budgetMultiplier, allocPct: pct, orderId: order.uuid });
    } catch (err) {
