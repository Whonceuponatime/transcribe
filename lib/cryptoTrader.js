...
function evaluateDipSignals(sig, score) {
  return [
    ...
    { reason: 'DIP_HIGH_SCORE',       cooldown: 6, met: score >= 5,                                    val: String(score),                                          thresh: 'score ≥ 5'      },
...
async function runDipBuys(supabase, config, priceMap, analysis, availableKrw, bear, {
  ...
  for (const coin of coins) {
    ...
    if (score < 2 && !hasOversoldSignal && !hasPullbackSignal && !hasRelaxedSignal) {
      console.log(`    ✗ SKIP — score ${score} below gate (min 2), no oversold/pullback/relaxed signal`);
      continue;
    }
    if (hasPullbackSignal && !hasOversoldSignal) {
      console.log(`    ✓ RSI pullback to ${sig.rsi?.toFixed(1)} — swing re-entry`);
    } else if (hasRelaxedSignal && !hasOversoldSignal && !hasPullbackSignal) {
      console.log(`    ✓ Over-cashed (${krwRatio?.toFixed(0)}% KRW) — relaxed re-entry RSI ${sig.rsi?.toFixed(1)}`);
    }
