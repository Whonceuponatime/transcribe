...
/** Persist a fill row to v2_fills. Logs on failure — a missing fill is auditable. */
async function persistFill(supabase, fillData) {
  try {
    const { error } = await supabase.from('v2_fills').insert(fillData);
    if (error) console.error(`[exec] persistFill DB error for ${fillData?.asset}:`, error.message);
  } catch (err) {
