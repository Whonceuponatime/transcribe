  ...
  try {
    ...
    for (const coin of coins) {
      try {
        ...
        const candles = await upbit.getCandleData(`KRW-${coin}`, 240, 120).catch(() => []);
        if (candles.length > 30) {
          const closes  = candles.map((c) => c.close ?? c.trade_price);
          const highs   = candles.map((c) => c.high  ?? c.high_price);
          const lows    = candles.map((c) => c.low   ?? c.low_price);
          const volumes = candles.map((c) => c.volume ?? c.candle_acc_trade_volume ?? 0);
    ...
    if (!opts.dipBuyOnly) {
      ...
      for (const position of openPositions) {
        ...
        const { data: peakRow } = await supabase.from('app_settings').select('value').eq('key', peakKey).single().catch(() => ({ data: null }));
        const peakPrice = peakRow?.value?.price ?? ind.currentPrice;

        // Update peak
        if (ind.currentPrice > peakPrice) {
          await supabase.from('app_settings').upsert({ key: peakKey, value: { price: ind.currentPrice, updated_at: new Date().toISOString() }, updated_at: new Date().toISOString() }, { onConflict: 'key' }).catch(() => {});
        }
        ...
        const exit = exits[0];
        const gainPct = ind.currentPrice && position.avg_cost_krw
        ...
        if (result.ok && !result.paper && !result.shadow) {
          ...
          if (exit.trim && !['time_stop', 'regime_break', 'runner'].includes(exit.trim)) {
            ...
            await supabase.from('positions').update({ fired_trims: firedTrims, updated_at: new Date().toISOString() }).eq('position_id', position.position_id).catch(() => {});
          }
          // If this was an adopted position's first bot-managed sell, promote it to 'open'
          if (position.state === 'adopted') {
            await adopter.promoteAdoptedPosition(supabase, position.position_id);
            console.log(`[v2] Adopted position ${coin} promoted to open after first managed exit`);
          }
        }
        // Paper/shadow mode — still promote adopted positions so they enter normal management
        if ((result.paper || result.shadow) && position.state === 'adopted') {
          await adopter.promoteAdoptedPosition(supabase, position.position_id);
        }
      }
    }
