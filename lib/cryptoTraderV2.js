...
async function executeCycleV2(supabase, opts = {}) {
  ...
  try {
    ...
    // ── 0c. Macro gate — Fear & Greed (fresh/starter entries only) ───────────
    // Read cached F&G from app_settings (written hourly by pi-trader).
    // Fails open: if absent, stale, or DB error → gate inactive, no blocking.
    // Never blocks: sells, add-ons, regime exits, trailing stops, loss-streak logic.
    let fearGreedValue    = null;
    let macroExtremeGreed = false;
    try {
      const { data: fgRow } = await supabase.from('app_settings').select('value').eq('key', 'fear_greed').single();
      const fgVal  = fgRow?.value?.value;
      const fgTime = fgRow?.value?.fetchedAt;
      if (fgVal != null && fgTime) {
        const ageH   = (Date.now() - new Date(fgTime).getTime()) / 3600000;
        const maxAge = cfg.macro_fg_stale_hours  ?? 24;
        const maxFg  = cfg.macro_fear_greed_max  ?? 75;
        if (ageH < maxAge) {
          fearGreedValue    = fgVal;
          macroExtremeGreed = fgVal > maxFg;
        }
      }
    } catch (_) {}
    ...
    // New-position cap per cycle: counts both fresh entries AND starter entries.
    // Add-ons (averaging into an existing position) are exempt — they don't open new risk.
    // Since coins = ['BTC', 'ETH', 'SOL'], BTC is evaluated first and wins naturally
    // when multiple signals fire in the same cycle.
    let freshEntriesThisCycle = 0;
    const maxFreshPerCycle = cfg.max_fresh_entries_per_cycle ?? 1;
    ...
    for (const coin of coins) {
      ...
      const intent = signalEngine.evaluateEntry(coin, regime, ind, cfg, portfolio.navKrw, cycleAdaptiveThresholds);
      ...
      if (!buysEnabled) {
        ...
        if (starterIntent) {
          // Starters count toward the per-cycle cap — they open new positions just like
          // fresh entries and are the main path for synchronized multi-coin entries.
          if (freshEntriesThisCycle >= maxFreshPerCycle) {
            buyBlocker = `max_fresh_entries_per_cycle:limit=${maxFreshPerCycle}`;
          } else if (macroExtremeGreed) {
            buyBlocker = `macro:extreme_greed:fg=${fearGreedValue}`;
          } else {
        ...
        // Fresh entry path — cap at maxFreshPerCycle to prevent all-three-at-once entries.
        // BTC fires first (first in coins[]), so it naturally gets priority.
        if (freshEntriesThisCycle >= maxFreshPerCycle) {
          buyBlocker = `max_fresh_entries_per_cycle:limit=${maxFreshPerCycle}`;
        } else if (macroExtremeGreed) {
          buyBlocker = `macro:extreme_greed:fg=${fearGreedValue}`;
      ...
      if (result.ok) {
        ...
        if (!isAddon) freshEntriesThisCycle++; // fresh + starter count; add-ons exempt
        if (cycleDecisions[coin]) {
