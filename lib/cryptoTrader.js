...
async function runDipBuys(supabase, config, priceMap, analysis, availableKrw, bear, {
  ...
  for (const coin of coins) {
    ...
    // When KRW > 40% of portfolio, relax gate — accept any mild pullback (RSI < 55)
    // so the bot actively redeploys accumulated cash rather than sitting idle
    const hasRelaxedSignal = overCashed && sig.rsi != null && sig.rsi < 55;
    if (score < 2 && !hasOversoldSignal && !hasRelaxedSignal) continue;
    if (hasRelaxedSignal && !hasOversoldSignal) {
      console.log(`[dip-buy] ${coin} — over-cashed (${krwRatio.toFixed(0)}% KRW) — accepting RSI ${sig.rsi?.toFixed(1)} < 55 as re-entry`);
    }
