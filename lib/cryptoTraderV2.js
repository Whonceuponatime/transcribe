...
async function executeCycleV2(supabase, opts = {}) {
  ...
  try {
    ...
    // Fresh-entry cap per cycle. Add-ons and starters are exempt — only first-position
    // entries count. Since coins = ['BTC', 'ETH', 'SOL'], BTC is evaluated first and
    // naturally wins when multiple signals fire in the same cycle.
    ...
    for (const coin of coins) {
      ...
      if (result.ok) {
        ...
        if (!isAddon && !isStarter) freshEntriesThisCycle++; // count toward per-cycle fresh cap
        if (cycleDecisions[coin]) {
