...
// Rate-limit CYCLE_FROZEN events to once per hour so they don't spam bot_events
// when the system is frozen across many cycles.
...
async function executeCycleV2(supabase, opts = {}) {
  ...
  try {
    ...
    if (reconEngine.isSystemFrozen()) {
      ...
      // Write CYCLE_FROZEN to bot_events at most once per hour.
      // This explains in the export why EXIT_EVALUATION, POSITION_SKIP_PROTECTED,
      // and EXECUTION events are absent — cycles never reached the sell/buy logic.
      const now = Date.now();
      if (now - _lastCycleFrozenLogAt >= CYCLE_FROZEN_LOG_INTERVAL_MS) {
        ...
        try {
          await supabase.from('bot_events').insert({
            ...
            message:      `V2 cycles frozen — no sell/buy logic running. freeze_reasons: ${reasons.slice(0,2).join(' | ')}`,
            context_json: {
              freeze_reasons:  reasons,
              mode,
              explanation:     'EXIT_EVALUATION, POSITION_SKIP_PROTECTED, and EXECUTION events will be absent until system is unfrozen. This is expected behaviour — no order logic runs while frozen.',
            },
            mode,
          });
        } catch (_) {}
    ...
    if (!opts.dipBuyOnly) {
      ...
      for (const position of openPositions) {
        ...
        // Log whenever a managed non-protected position reaches the sell evaluator.
        // Only emit when an exit fires OR when the position is above required edge
        // (i.e., it was genuinely evaluated for profit, not silently below threshold).
          ...
          try {
            await supabase.from('bot_events').insert({
              ...
              message:      exitFired
                ...
                : `${coin} above edge (${netGainPct?.toFixed(2)}%) — no trigger met`,
              context_json: {
                position_id:        position.position_id,
                symbol:             coin,
                strategy_tag:       position.strategy_tag,
