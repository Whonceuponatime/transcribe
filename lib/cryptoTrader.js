...
async function runSells(supabase, config, accounts, priceMap, analysis) {
  ...
  for (const coin of coins) {
    ...
    if (config.signal_sell_enabled && sig && !sig.error) {
      ...
      // ── Global signal-sell cooldown: 3h after ANY signal sell on this coin ──────
      // Without this, the bot rotates through RSI→BB→StochRSI→VWAP every 5 minutes,
      // selling a chunk each time. Now after any signal fires, all signals rest 3h.
      const recentSignalSell = await isRecentlySoldAny(supabase, coin, 3);
      if (recentSignalSell) {
        console.log(`[sell] ${coin} — signal sell cooldown active (sold in last 3h) — skipping all signals`);
        // Still allow profit-take tiers above (they have their own cooldown logic)
        // Jump straight to trailing stop check
      } else {
