...
// Profit-take levels: { gainPct threshold, DB label, % of holdings to sell }
const PROFIT_LEVELS = [
...
async function runProfitTake(supabase, config, accounts, priceMap) {
  ...
  for (const coin of coins) {
    ...
    for (const level of PROFIT_LEVELS) {
      if (gainPct < level.gainThreshold) continue;

      const alreadyTriggered = await isProfitTakeTriggered(supabase, coin, level.label, 3);
...
async function getStatus(supabase) {
  ...
  const positions = coins.map((coin) => {
    ...
    const nextProfitTake = gainPct != null
      ? PROFIT_LEVELS.find((l) => gainPct < l.gainThreshold) ?? null : null;
