  ...
  for (const coin of coins) {
    ...
    if (config.signal_sell_enabled && sig && !sig.error) {
      ...
      if (sellConditions >= 3 && !atProfit) {
        console.log(`[sell] ${coin} — ${sellConditions} sell signals BUT position not yet profitable (gain ${netGainPct != null ? netGainPct.toFixed(2) : '?'}% < ${MIN_PROFIT_TO_SELL}% min) — holding`);
