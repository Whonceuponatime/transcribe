  ...
  try {
    ...
    for (const order of toProcess) {
      try {
        ...
        if (posErr) {
          console.error(`[recon] backfillOrphanedFills: position update failed for ${order.asset}:`, posErr.message);
          failed.push({ id: order.id, asset: order.asset, error: posErr.message });
          continue;
        }

        // Insert fill records
        const fillRows = trades.length > 0 ? trades : [{ price: avgPrice, volume: executedVol, funds: executedVol * avgPrice, uuid: null, created_at: null }];
        for (const trade of fillRows) {
          try {
            await supabase.from('v2_fills').upsert({
              order_id:         order.id,
              position_id:      order.position_id,
              asset:            order.asset,
              side:             'sell',
            ...
            console.error(`[recon] backfillOrphanedFills: fill insert failed for ${order.asset}:`, fillErr.message);
          }
        }

        console.log(`[recon] backfillOrphanedFills: ${order.asset} — qty ${pos.qty_open}→${newQty} fired_trims=${JSON.stringify(firedTrims)}`);
        ...
        try {
          await supabase.from('bot_events').insert({
            ...
            severity:     'warn',
            subsystem:    'reconciliation',
            message:      `${order.asset} orphaned sell fill backfilled on startup`,
            context_json: {
              order_id:        order.id,
              asset:           order.asset,
              executed_volume: executedVol,
        ...
        applied.push({ id: order.id, asset: order.asset, qtyBefore: pos.qty_open, qtyAfter: newQty });
