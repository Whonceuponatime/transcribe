  ...
  try {
    ...
    for (const coin of coins) {
      ...
      const rsiThreshold = r === 'UPTREND'
        ...
        : `<${r === 'RANGE' ? (cfg.entry_rsi_max_range ?? 35) : (cfg.entry_rsi_max_downtrend ?? 28)}`;
      ...
      if (cycleDecisions[coin]) {
        ...
        if (buyEligible) {
          cycleDecisions[coin].final_action = 'BUY_ELIGIBLE';
          cycleDecisions[coin].final_reason = `buy:${intent?.reason ?? 'signal_met'}`;
        } else {
      ...
      if (result.ok) {
        if (cycleDecisions[coin]) {
          cycleDecisions[coin].final_action = 'BUY_SUBMITTED';
          cycleDecisions[coin].final_reason = `buy_order:${result.state}`;
        }
        for (const fill of (result.fills || [])) {
