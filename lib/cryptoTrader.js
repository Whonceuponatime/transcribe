...
function dcaCooldownDays(avgScore, fng = null, configDays = 1) {
  ...
  // Fear — respect user setting (already likely 1)
  if (fng != null && fng < 30) days = Math.min(days, configDays);
  // Strong signals shorten cooldown slightly
  if (avgScore >= 6) days = Math.min(days, configDays);
  return Math.max(days, 1); // never less than 1 day
}
...
async function runDipBuys(supabase, config, priceMap, analysis, availableKrw, bear, {
  ...
  for (const coin of coins) {
    ...
    if (recentlySold) {
      console.log(`[dip-buy] ${coin} sold in last 30min — skipping to avoid fee churn`);
      continue;
    }
    ...
    // RSI pullback: RSI cooled from overbought back to neutral (30-50) — swing re-entry
    // This is the key signal for "sell high, rebuy the pullback, repeat" cycle
    const hasPullbackSignal = sig.rsi != null && sig.rsi < 50 && sig.rsi >= 30;
    // When over-cashed also accept RSI < 60 to more aggressively redeploy
    const hasRelaxedSignal  = overCashed && sig.rsi != null && sig.rsi < 60;
    if (score < 2 && !hasOversoldSignal && !hasPullbackSignal && !hasRelaxedSignal) continue;
    if (hasPullbackSignal && !hasOversoldSignal) {
      console.log(`[dip-buy] ${coin} — RSI pullback to ${sig.rsi?.toFixed(1)} — swing re-entry`);
    } else if (hasRelaxedSignal && !hasOversoldSignal && !hasPullbackSignal) {
      console.log(`[dip-buy] ${coin} — over-cashed (${krwRatio.toFixed(0)}% KRW) — relaxed re-entry RSI ${sig.rsi?.toFixed(1)}`);
    }
    ...
    // DIP_RSI_PULLBACK: the key swing-trade signal.
    // After selling at RSI>75, RSI naturally drops to 45-55 during the next wave.
    // Buying there lets the bot re-enter before the next rally instead of waiting
    // for an extreme crash (RSI<30) that may never come in an uptrend.
    const dipSignals = [
      ...
      // Pullback re-entry: RSI cooled from overbought back to neutral — rebuy the dip
      // Buy size is halved vs a full dip signal so we don't over-deploy on mild pulls
      sig.rsi != null && sig.rsi < 50 && sig.rsi >= 30
        ? { reason: 'DIP_RSI_PULLBACK', cooldown: 4, sizeMult: 0.5 } : null,
    ].filter(Boolean);

    let executed = false;
    for (const { reason, cooldown, sizeMult = 1 } of dipSignals) {
      if (executed) break;
      ...
      if (thisAmount < MIN_ORDER_KRW) continue;
      ...
      try {
        ...
        console.log(`[dip-buy] ${coin} ${reason}: ₩${thisAmount.toLocaleString()} − fee ₩${feeKrw.toLocaleString()} = ₩${coinKrw.toLocaleString()} in coin (score ${score}, mult ${(mult * sizeMult).toFixed(2)}×)`);
