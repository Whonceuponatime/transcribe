...
/** Clear in-memory per-asset entry state when a position is fully closed (add-on cap + adaptive RSI). */
function clearPerAssetEntryState(coin) {
...
async function executeCycleV2(supabase, opts = {}) {
  ...
  try {
    ...
    // ── 6. Sell cycle ─────────────────────────────────────────────────────────
    let openPositions = await getOpenPositions(supabase);
    if (!opts.dipBuyOnly) {
      ...
      for (const position of openPositions) {
        ...
        if (result.ok && !result.paper && !result.shadow) {
          ...
          if (['core_final_exit','core_time_stop','tactical_final_exit','tactical_time_stop','ladder_exhausted_exit'].includes(exit.trim)) {
            const cooldownMs = cfg.core_exit_reentry_cooldown_ms ?? 4 * 60 * 60 * 1000;
            const cooldownUntil = Date.now() + cooldownMs;
            _coreExitCooldownUntil.set(coin, cooldownUntil);
            ...
            try {
              await supabase.from('bot_events').insert({
                event_type:   'REENTRY_COOLDOWN_SET',
                severity:     'info',
                subsystem:    'sell_cycle',
                message:      `${coin} re-entry blocked until ${new Date(cooldownUntil).toISOString()}`,
                context_json: {
                  symbol:            coin,
                  cooldown_until_iso: new Date(cooldownUntil).toISOString(),
          ...
          // Reset add-on + adaptive RSI only after DB confirms full close (avoids relying
          // on sellPct alone and matches tactical_final_exit / any 100% exit once qty=0).
          let positionFullyClosed = false;
          try {
            ...
            const st       = refreshedPos?.state ?? null;
            positionFullyClosed =
              st === 'closed'
              || (qtyAfter != null && qtyAfter <= 0)
              || (refreshedPos == null && exit.sellPct >= 100);
            console.log(`[v2] POSITION_REFRESH_AFTER_SELL: ${coin} qty_before=${Number(position.qty_open)} qty_after=${qtyAfter ?? 'unknown'} state=${st ?? 'unknown'} fully_closed=${positionFullyClosed}`);
            await supabase.from('bot_events').insert({
              ...
              context_json: {
                ...
                qty_after:      qtyAfter,
                position_state: st,
                fully_closed:   positionFullyClosed,
                order_id:       result.orderId,
                exit_trim:      exit.trim,
                engine:         'V2',
                execution_mode: EXECUTION_MODE,
            ...
            positionFullyClosed = exit.sellPct >= 100;
          }
          if (positionFullyClosed) {
            clearPerAssetEntryState(coin);
          }
    ...
    // Positions can change during the sell loop (fills / fallback updates). Re-fetch
    // before buys so the same cycle cannot treat a just-closed row as still open.
    openPositions = await getOpenPositions(supabase);
    ...
    // ── Adaptive reserve sizing ─────────────────────────────────────────────
    const _baseReservePct = cfg.krw_min_reserve_pct ?? 20;
    const _regimeStr = regime?.regime ?? 'RANGE';
    const _adxVal = regime?.adxVal ?? 0;

    let _reserveRegimeAdj = 0;
    if (_regimeStr === 'UPTREND' && _adxVal >= 40)  _reserveRegimeAdj = -5;
    else if (_regimeStr === 'UPTREND' && _adxVal < 25) _reserveRegimeAdj = +5;
    else if (_regimeStr === 'RANGE')                   _reserveRegimeAdj = +8;
    else if (_regimeStr === 'DOWNTREND')               _reserveRegimeAdj = +15;

    let _fgValue = 50;
    try {
      const { data: fgRow } = await supabase.from('app_settings').select('value').eq('key', 'fear_greed').single();
      if (fgRow?.value?.value != null) _fgValue = Number(fgRow.value.value);
    } catch (_) {}

    let _reserveFgAdj = 0;
    if (_fgValue <= 20)      _reserveFgAdj = -5;
    else if (_fgValue <= 35) _reserveFgAdj = -2;
    else if (_fgValue >= 90) _reserveFgAdj = +10;
    else if (_fgValue >= 75) _reserveFgAdj = +5;

    const _lossStreakCount = _riskState.lossStreak ?? 0;
    let _reserveStreakAdj = 0;
    if (_lossStreakCount >= 5)      _reserveStreakAdj = +10;
    else if (_lossStreakCount >= 3) _reserveStreakAdj = +5;

    const effectiveReservePct = Math.min(
      Math.max(_baseReservePct + _reserveRegimeAdj + _reserveFgAdj + _reserveStreakAdj, 10),
      45
    );
    const reserveKrw   = portfolio.navKrw * (effectiveReservePct / 100);
    const freeKrwAvail = Math.max(0, portfolio.krwBalance - reserveKrw);

    console.log(`[v2] effective_reserve=${effectiveReservePct}% (base=${_baseReservePct} regime=${_reserveRegimeAdj > 0 ? '+' : ''}${_reserveRegimeAdj} fg=${_reserveFgAdj > 0 ? '+' : ''}${_reserveFgAdj} streak=${_reserveStreakAdj > 0 ? '+' : ''}${_reserveStreakAdj}) floor=₩${Math.round(reserveKrw).toLocaleString()}`);
    ...
    for (const coin of coins) {
      ...
      let intent = signalEngine.evaluateEntry(coin, regime, ind, cfg, portfolio.navKrw, thresholdsWithRsi);

      // Existing row for this asset (re-fetched after sell cycle so qty/state match DB).
      ...
      // ── NAV-proportional entry sizing ──────────────────────────────────────
      // Size to 90% of each coin's hard exposure cap so capital isn't stranded
      // behind the old target_deployment_pct ceiling.
      const targetDeploymentPct   = cfg.target_deployment_pct ?? 30; // kept for reference/logging only
      const targetEntriesPerPos   = cfg.target_entries_per_position ?? 8;
      const exposureCapPct        = coin === 'BTC'
        ? (cfg.max_btc_pct ?? 60)
        : coin === 'ETH'
        ? (cfg.max_eth_pct ?? 45)
        : coin === 'XRP'
        ? (cfg.max_xrp_pct ?? 35)
        : (cfg.max_sol_pct ?? 35);
      const maxPerAssetKrw        = portfolio.navKrw * (exposureCapPct / 100) * 0.90;
      const currentExposureKrw    = posNotionalKrw;
      const remainingForAssetKrw  = Math.max(0, maxPerAssetKrw - currentExposureKrw);
      const navEntrySize          = Math.max(0, remainingForAssetKrw / Math.max(1, targetEntriesPerPos));
      console.log(`[v2] nav_sizing: ${coin} maxPerAsset=₩${Math.round(maxPerAssetKrw).toLocaleString()} (exposure_cap=${exposureCapPct}% ×0.90) remaining=₩${Math.round(remainingForAssetKrw).toLocaleString()} entrySize=₩${Math.round(navEntrySize).toLocaleString()}`);

      // Override signal-engine sizing on the fresh pullback intent.
      // evaluateEntry still controls signal/no-signal; only the KRW amount changes.
      if (intent && navEntrySize >= 5000) {
        intent.krwAmount = navEntrySize;
      } else if (intent && navEntrySize < 5000) {
        intent = null; // remaining room too small to deploy
        buyBlocker = `nav_sizing:remaining_for_asset=${Math.round(remainingForAssetKrw)}_below_min`;
      }
      ...
      if (!buysEnabled) {
      ...
      } else if (gatingPos && (intent || gatingPos.strategy_tag === 'tactical') && (_addonBuyCount.get(coin) ?? 0) >= (cfg.max_addons_per_position ?? 6)) {
        // Only block add-on-type entries (pullback intent = real add-on, tactical gatingPos = starter-into-existing).
        // Fresh starters on a flat/micro portfolio are not add-ons and should not be capped.
        ...
        gatingPos &&
        (intent || gatingPos.strategy_tag === 'tactical') &&
        (() => {
          // Block new add-on / starter-into-existing buys on zombie positions:
          // all four primary rungs fired AND idle >24h since last fill.
          // These positions should be winding down via ladder_exhausted_exit
          // (or tactical/core final exit), not accumulating more dust entries.
          const ft = gatingPos.fired_trims ?? [];
          const allPrimary = ft.includes('trim1') && ft.includes('trim2')
            && ft.includes('post_trim_runner') && ft.includes('runner_protect');
          if (!allPrimary) return false;
          const lastFillAt = gatingPos.updated_at ?? gatingPos.updatedAt;
          if (!lastFillAt) return false;
          const idleHours = (Date.now() - new Date(lastFillAt).getTime()) / 3_600_000;
          const idleCap   = cfg.buy_block_ladder_exhausted_hours ?? 24;
          return idleHours >= idleCap;
        })()
      ) {
        const lastFillAt = gatingPos.updated_at ?? gatingPos.updatedAt;
        const idleHours  = lastFillAt ? ((Date.now() - new Date(lastFillAt).getTime()) / 3_600_000) : 0;
        buyBlocker = `ladder_exhausted:all_primary_fired_idle_${idleHours.toFixed(1)}h`;
        console.log(`[buy][${coin}] skipped — ladder_exhausted: all 4 primary rungs fired, idle ${idleHours.toFixed(1)}h since last fill`);
        ...
        if (starterAddonCandidate) {
          ...
          const scaledKrw         = navEntrySize * starterAddonMult;
        ...
        if (starterIntent) {
          starterIntent.krwAmount = navEntrySize; // NAV-proportional override
          const lastBuy      = _lastBuyAt.get(coin) ?? 0;
          ...
          if (r === 'DOWNTREND') {
            ...
            if (dtReclaim) {
              dtReclaim.krwAmount = navEntrySize; // NAV-proportional override
              const lastBuy    = _lastBuyAt.get(coin) ?? 0;
      ...
      if (cycleDecisions[coin]) {
        cycleDecisions[coin].buy_checks = {
          ...
          effective_reserve_pct: effectiveReservePct,
          reserve_regime_adj:    _reserveRegimeAdj,
          reserve_fg_adj:        _reserveFgAdj,
          reserve_streak_adj:    _reserveStreakAdj,
          reserve_fg_value:      _fgValue,
          ...
          // ── NAV-proportional sizing diagnostics ────────────────────────────────
          nav_entry_size:         +navEntrySize.toFixed(0),
          max_per_asset_krw:      +maxPerAssetKrw.toFixed(0),
          current_exposure_krw:   +currentExposureKrw.toFixed(0),
          remaining_for_asset_krw: +remainingForAssetKrw.toFixed(0),
          exposure_cap_pct:       exposureCapPct,
          target_deployment_pct:  targetDeploymentPct,
          target_entries_per_pos: targetEntriesPerPos,
        };
      ...
      if (result.ok) {
        ...
        const isExplicitAddon = result.entryReason?.includes('_into_existing') || result.entryReason?.includes('_addon') || isAddon;
        if (isExplicitAddon) {
...
function restoreCooldown(coin, untilMs) {
  _coreExitCooldownUntil.set(coin, untilMs);
}

module.exports = { executeCycleV2, getV2Config, restoreCooldown };
