    ...
    for (const order of stuck) {
      try {
        ...
        if (order.side === 'sell' && order.position_id && executedVol > 0) {
          ...
          console.log(`[recon] resolveStuckOrders: ${order.asset} ${order.side} settled as ${dbState} (no position update needed)`);
...
async function backfillOrphanedFills(supabase) {
  ...
  try {
    // Find sell orders in terminal state with no v2_fills rows
    ...
    console.log(`[recon] backfillOrphanedFills: ${toProcess.length} orphaned sell order(s) need fill backfill`);
    ...
    for (const order of toProcess) {
      try {
        ...
        // ── Update position (best-effort — fill insert runs regardless) ─────
        // Do NOT gate fill insert on posErr. The fill record is an audit trail
        // that must be written even if the position is already correct (e.g.
        // already reduced to ~0 by FILL_FALLBACK_DIRECT over prior cycles).
        ...
        if (posErr) {
          console.error(`[recon] backfillOrphanedFills: position update failed for ${order.asset} (non-fatal — fill insert proceeds):`, posErr.message);
          // Do NOT continue — fill insert and event must still run
        }

        // ── Insert synthetic fill record ─────────────────────────────────
        // Do NOT include upbit_trade_uuid here — migration 031 may not have
        // been applied yet, and a missing column causes a silent Supabase
        // error (PostgREST returns {error}, does not throw). Idempotency is
        // guaranteed by the existingFillCount check above, not by the column.
        // One synthetic fill per order — uses executedVol and avgPrice.
        ...
        if (fillInsertErr) {
          console.error(`[recon] backfillOrphanedFills: fill insert failed for ${order.asset} (${order.id}):`, fillInsertErr.message);
          // Still emit the event — the audit gap should be visible in bot_events
        } else {
          console.log(`[recon] backfillOrphanedFills: ${order.asset} fill inserted — qty=${fillQty.toFixed(8)} price=${fillPrice}`);
        }

        console.log(`[recon] backfillOrphanedFills: ${order.asset} — pos ${pos.qty_open}→${newQty} posErr=${posErr?.message ?? 'none'} fillErr=${fillInsertErr?.message ?? 'none'}`);

        // ── Emit audit event — log error explicitly (no silent catch) ───
        ...
        applied.push({ id: order.id, asset: order.asset, qtyBefore: pos.qty_open, qtyAfter: newQty, fillInserted: !fillInsertErr });
