...
const CANDLES_NEEDED = 220;                // 220 × 4h ≈ 36 days — enough for EMA200 + ADX
...
async function getRegime(supabase, cfg = {}) {
  ...
  let candles;
  try {
    candles = await upbit.getCandleData('KRW-BTC', 240, CANDLES_NEEDED);
  } catch (err) {
    console.error('[regime] Failed to fetch candles:', err.message);
    // Return cached (even if stale) rather than crash the cycle
