...
async function runAdoption(supabase, supportedCoins = ['BTC', 'ETH', 'SOL'], mode = 'paper', force = false) {
  ...
  try {
    ...
    for (const acc of accounts) {
      ...
      // Normalise symbol
      const symbol = normalizeSymbol(acc.currency);
      if (!symbol) {
        console.warn(`[adoption] Unmappable symbol: ${acc.currency} — skipping`);
        skipped.push({ currency: acc.currency, reason: 'symbol_normalization_failed' });
        continue;
      }
      ...
      // Attempt for supported coins only to avoid unnecessary API calls
      let markPrice = supportedCoins.includes(symbol) ? await fetchMarkPrice(symbol) : null;
      if (markPrice == null && avgBuyKrw > 0) markPrice = avgBuyKrw; // fallback
      ...
      // ── 4. Classify: supported vs unsupported ────────────────────────────
