...
async function updateOrderState(supabase, id, patch) {
  ...
  try {
    ...
    console.error(`[exec] updateOrderState failed for order ${id}:`, err.message);
  }
}

/**
 * Persist a fill row to v2_fills with idempotent conflict handling.
 *
 * Uses upsert with onConflict='upbit_trade_uuid' + ignoreDuplicates so that
 * re-inserting the same real trade (same upbit_trade_uuid) is silently skipped
 * rather than erroring. The DB unique index on upbit_trade_uuid enforces this.
 *
 * For synthetic fills (upbit_trade_uuid IS NULL), Postgres NULLs don't conflict
 * on the unique index — a second synthetic fill for the same order_id will be
 * caught by the partial unique index on (order_id) WHERE upbit_trade_uuid IS NULL,
 * which returns a DB error that is logged below.
 */
async function persistFill(supabase, fillData) {
  try {
    ...
    if (error) console.error(`[exec] persistFill DB error for ${fillData?.asset} trade=${fillData?.upbit_trade_uuid}:`, error.message);
  } catch (err) {
    console.error(`[exec] persistFill failed for ${fillData?.asset}:`, err.message);
  }
}
 ...
 *
 * One row is created per Upbit trade (t.uuid is the natural idempotency key).
 * upbit_trade_uuid is stored so the DB unique index can prevent duplicate inserts
 * if this function is called more than once for the same settled order.
 */
function extractFills(exchangeResp, orderId, positionId, strategyTag, entryContext) {
  ...
  return trades.map((t) => ({
    ...
    upbit_trade_uuid: t.uuid ?? null,          // Upbit per-trade UUID — idempotency key
...
async function executeBuy(supabase, intent, regime, context = {}) {
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      // ── Poll for settlement if Upbit returned wait/watch ─────────────────
      // Market buy orders frequently return state:'wait' on the immediate
      // response with an empty trades[] array. Without polling, extractFills
      // returns [] and the linked position (created with qty_open=0) is never
      // updated — leaving a zombie open position and an accepted order that
      // reconciliation will freeze on. Mirror the sell-side polling behaviour.
      if (resp.state === 'wait' || resp.state === 'watch') {
        console.log(`[exec] BUY ${asset} initial response state:${resp.state} — polling for settlement (identifier: ${identifier})`);
        ...
        if (settled) {
          ...
          console.warn(`[exec] BUY ${asset} poll timed out — order may remain accepted; startup resolveStuckOrders will repair on next restart`);
        }
      }
