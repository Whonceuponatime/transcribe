 ...
 * Fires once (tactical only) after trim1 + post_trim_runner have both fired,
...
function evaluateExit(position, ind, regime, feeRate, cfg, peakPrice) {
  ...
  // Tactical positions only. Fires once after trim1 + post_trim_runner have both
  // fired, when either:
  ...
  // Prevents above-edge tactical leftovers from sitting at above_edge_no_exit_condition_met
  // while the position quietly loses its realised edge.
