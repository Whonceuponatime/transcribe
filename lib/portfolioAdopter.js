...
async function runAdoption(supabase, supportedCoins = ['BTC', 'ETH', 'SOL'], mode = 'paper', force = false) {
  ...
  if (!force) {
    ...
    if (existing) {
      ...
      try {
        const { data: currentPositions } = await supabase.from('positions')
        ...
        await supabase.from('bot_events').insert({
          ...
          message:      `Adoption previously completed (run ${existing.id}) — ${adoptedPositions.length} adopted positions active, ${protectedCount} still unassigned`,
          context_json: {
