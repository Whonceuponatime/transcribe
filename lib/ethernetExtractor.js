...
async function extractPdfText(pdfBuffer) {
  ...
  try {
    ...
    const byPage = fullText.split(/\f+/).filter(Boolean);
    numPages = Math.max(1, result.total ?? result.numpages ?? byPage.length);
  } finally {
    if (typeof parser.destroy === 'function') parser.destroy();
  }

  // pdf-parse may not give per-page text; split by form-feed or approximate
  const pages = [];
