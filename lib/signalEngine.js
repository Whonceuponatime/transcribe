...
const SAFETY_BUFFER_PCT   = 0.20;   // 0.20% above break-even
...
async function getCandles4h(asset, count = 100) {
  const candles = await upbit.getCandleData(`KRW-${asset}`, 240, count);
  return {
    closes:  candles.map((c) => c.close  ?? c.trade_price),
    highs:   candles.map((c) => c.high   ?? c.high_price),
    lows:    candles.map((c) => c.low    ?? c.low_price),
    volumes: candles.map((c) => c.volume ?? c.candle_acc_trade_volume ?? 0),
    raw:     candles,
  };
}
...
function evaluateEntry(asset, regime, ind, cfg, navKrw) {
  ...
  const obMin = cfg.ob_imbalance_min ?? -0.30;
  if (imbal != null && imbal < obMin) {
    return null; // order book too negative — bad fill quality
  }
  ...
  if (r === 'UPTREND') {
    const bbThresh  = cfg.entry_bb_pct_uptrend   ?? 0.25;
    ...
    if (met) {
      ...
      sizePct = 50; // 50% of budget on first entry; second add possible
    }

  } else if (r === 'RANGE') {
    const bbThresh = cfg.entry_bb_pct_range   ?? 0.10;
    const rsiMax   = cfg.entry_rsi_max_range  ?? 35;
...
function evaluateExit(position, ind, regime, feeRate, cfg, peakPrice) {
  ...
  const roundTrip = (feeRate * 2) * 100; // e.g. 0.50%
  const reqEdge   = roundTrip + (cfg.exit_safety_buffer_pct ?? SAFETY_BUFFER_PCT);

  // Net gain after fees
  const netGainPct = gainPct - roundTrip;

  // ATR-based thresholds
  const atrPct     = ind.atrPct ?? 1.0; // fallback 1%
  const trim1Mult  = cfg.exit_atr_trim1    ?? 1.20;
  const trim2Mult  = cfg.exit_atr_trim2    ?? 2.00;
  const trailMult  = cfg.exit_atr_trailing ?? 1.50;
  const timeStopH  = cfg.exit_time_stop_hours ?? 30;

  const trim1Target   = reqEdge + trim1Mult * atrPct;
  const trim2Target   = trim2Mult * atrPct;
  const trailDropPct  = trailMult * atrPct;

  const heldHours  = (Date.now() - new Date(opened_at).getTime()) / 3600000;
  const isAdopted  = position.state === 'adopted';
  // Adopted positions are protected from forced regime-break exits for 24h.
  // This prevents the bot from immediately liquidating pre-existing holdings
  // that were imported at startup and may not align with the current regime.
  // After 24h the position is treated as a normal open position.
  const adoptionGraceHours = 24;
  const inAdoptionGrace    = isAdopted && heldHours < adoptionGraceHours;

  if (inAdoptionGrace) {
    console.log(`    [adoption grace] ${asset} adopted ${heldHours.toFixed(1)}h ago — regime-break exits suppressed for ${(adoptionGraceHours - heldHours).toFixed(1)}h more`);
  }

  // ── Regime break — cut tactical SOL immediately if BTC turns DOWNTREND ────
  // Adopted SOL positions get a 24h grace period before forced regime-break exit.
  if (regime.regime === 'DOWNTREND' && asset === 'SOL') {
    if (inAdoptionGrace) {
      // Still log a warning so operator is aware
      console.log(`    [adoption grace] ${asset} would be cut by regime_break_sol but is in adoption grace period`);
    } else {
  ...
  // ── Regime break — halve BTC/ETH tactical on downtrend flip ──────────────
  if (regime.regime === 'DOWNTREND' && (asset === 'BTC' || asset === 'ETH')) {
    if (position.entry_regime !== 'DOWNTREND' && !inAdoptionGrace) {
      exits.push({ asset, side: 'sell', sellPct: 50, reason: 'regime_break_reduce', trim: 'regime_break' });
      return exits;
    }
    if (inAdoptionGrace) {
      console.log(`    [adoption grace] ${asset} would be halved by regime_break_reduce but is in adoption grace period`);
    }
  }
  ...
  // Only consider profit exits if above minimum required edge
  if (netGainPct < reqEdge) return exits;

  // Track which trims have already fired using position metadata
  const firedTrims = position.fired_trims ?? [];

  // ── Trim 1 (25%) — quick risk reduction at +1.2 ATR ──────────────────────
  if (!firedTrims.includes('trim1') && gainPct >= trim1Target) {
    exits.push({ asset, side: 'sell', sellPct: 25, reason: `trim1_${gainPct.toFixed(1)}pct`, trim: 'trim1' });
  }

  // ── Trim 2 (35%) — larger move captured at +2.0 ATR or RSI overbought ────
  if (!firedTrims.includes('trim2') && firedTrims.includes('trim1')) {
    const rsiOB = ind.rsi14 != null && ind.rsi14 > 65 && ind.ema20 != null && currentPrice > ind.ema20;
    if (gainPct >= trim2Target || rsiOB) {
      exits.push({ asset, side: 'sell', sellPct: 35, reason: `trim2_${gainPct.toFixed(1)}pct`, trim: 'trim2' });
    }
  }

  // ── Runner trailing stop (40%) — protect remaining after trim2 ───────────
