...
function evaluateExit(position, ind, regime, feeRate, cfg, peakPrice) {
  ...
  // ── Ladder exhausted exit — lenient zombie-rescue fallback ────────────────
  // Fires when all four primary rungs (trim1, trim2, post_trim_runner,
  // runner_protect) have fired and the position has been idle a long time.
  // Lower profit threshold than tactical_final_exit / core_final_exit so a
  // position doesn't zombie forever when net gain is stuck just below 0.50%.
  // Two firing conditions (either qualifies):
  //   (a) 24h+ since last fill AND net >= 0.30%  — bank a modest win
  //   (b) 48h+ since last fill AND net >  0.00%  — break even and move on
  // Never forces a realized loss — waits for net to tick above zero.
  // Guard: fires only once (ladder_exhausted_exit in fired_trims).
  ...
  if (
    !ladderExhaustedFired &&
    allPrimaryRungsFired &&
    hoursSinceUpdateLE != null &&
    (
      (hoursSinceUpdateLE >= ladderExhaustedHours     && netGainPct >= ladderExhaustedMinNet) ||
      (hoursSinceUpdateLE >= ladderExhaustedLateHours && netGainPct >  0.0)
    )
  ) {
    exits.push({
      asset,
