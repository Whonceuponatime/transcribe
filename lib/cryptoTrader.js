...
async function getConfig(supabase) {
  ...
  return {
    ...
    // Capital % mode — budget scales automatically with KRW balance
    capital_pct_mode:         data?.capital_pct_mode ?? false,
  ...
  for (const coin of coins) {
    ...
    if (config.signal_sell_enabled && sig && !sig.error) {
      ...
      if (sig.rsi != null && sig.rsi > 80 && atProfit) {
        ...
        if (!done) results.push(await sellCoin(supabase, coin, balance, currentPrice, scaleSell(20), 'SIGNAL_RSI_OB_STRONG', 'rsi_ob_strong', { rsi: sig.rsi.toFixed(1) }));
      } else if (sig.rsi != null && sig.rsi > 70 && atProfit) {
        const done = await isRecentlySold(supabase, coin, 'rsi_ob', 2);
        if (!done) results.push(await sellCoin(supabase, coin, balance, currentPrice, scaleSell(12), 'SIGNAL_RSI_OVERBOUGHT', 'rsi_ob', { rsi: sig.rsi.toFixed(1) }));
      }

      // 3. Bollinger upper breakout
      if (sig.bb?.pctB > 1.0 && atProfit) {
        const done = await isRecentlySold(supabase, coin, 'bb_upper', 2);
        if (!done) results.push(await sellCoin(supabase, coin, balance, currentPrice, scaleSell(12), 'SIGNAL_BB_UPPER', 'bb_upper', { pctB: sig.bb.pctB.toFixed(2) }));
      }

      // 4. MACD bearish cross
      if (sig.macd?.bearishCross) {
        const done = await isRecentlySold(supabase, coin, 'macd_bear', 4);
        if (!done) results.push(await sellCoin(supabase, coin, balance, currentPrice, scaleSell(10), 'SIGNAL_MACD_BEAR', 'macd_bear'));
      }
      ...
      if (sig.stochRsi != null && sig.stochRsi > 85 && atProfit) {
        ...
        if (!done) results.push(await sellCoin(supabase, coin, balance, currentPrice, scaleSell(10), 'SIGNAL_STOCHRSI_OB', 'stochrsi_ob', { stochRsi: sig.stochRsi.toFixed(1) }));
      }
      ...
      if (sig.vwapDev != null && sig.vwapDev > 3 && atProfit) {
        ...
        if (!done) results.push(await sellCoin(supabase, coin, balance, currentPrice, scaleSell(12), 'SIGNAL_VWAP_ABOVE', 'vwap_above', { vwapDev: sig.vwapDev.toFixed(1) }));
      }
      ...
      if (sig.williamsR != null && sig.williamsR > -15 && atProfit) {
        ...
        if (!done) results.push(await sellCoin(supabase, coin, balance, currentPrice, scaleSell(10), 'SIGNAL_WILLIAMS_OB', 'williams_ob', { wR: sig.williamsR.toFixed(1) }));
      }

      // 8. CCI overbought
      if (sig.cci != null && sig.cci > 150 && atProfit) {
        const done = await isRecentlySold(supabase, coin, 'cci_ob', 2);
        if (!done) results.push(await sellCoin(supabase, coin, balance, currentPrice, scaleSell(10), 'SIGNAL_CCI_OB', 'cci_ob', { cci: sig.cci.toFixed(0) }));
      }
      ...
      if (sig.kimchiPremium != null && sig.kimchiPremium > 4 && atProfit) {
        ...
        if (!done) results.push(await sellCoin(supabase, coin, balance, currentPrice, scaleSell(15), 'SIGNAL_KIMCHI_HIGH', 'kimchi_high', { kimchi: sig.kimchiPremium.toFixed(1) }));
      }
...
async function runDipBuys(supabase, config, priceMap, analysis, availableKrw, bear, {
  totalPortfolioKrw = 0, portfolioWeights = {},
} = {}) {
  const results = [];
  ...
  for (const coin of coins) {
    ...
    // ── Score gate: only buy when at least 1 confirmed buy signal ─────────────
  ...
  if (config.dip_buy_enabled && (fearGreed?.value ?? 50) <= 75) {
    try {
      summary.dipBuys = await runDipBuys(supabase, config, priceMap, analysis, krwBalance, bear, {
        totalPortfolioKrw, portfolioWeights,
      });
  ...
  if (config.dca_enabled && !dipBuyOnly) {
    ...
    if (forceDca || daysSinceLast >= cooldown) {
      ...
      try {
        ...
        summary.dca = await runDca(supabase, config, signalScore, fearGreed, priceMap, bear, freshAccounts, analysis);
