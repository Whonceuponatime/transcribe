 ...
 *   strategy_tag = 'tactical' — auto-classified on creation so sell cycle works immediately
 ...
 * }}
 */
async function runAdoption(supabase, supportedCoins = ['BTC', 'ETH'], mode = 'paper', force = false) {
  ...
  if (!force) {
    ...
    if (existing) {
      ...
      // Fetch current position state so the audit trail is never empty on restart.
      // ADOPTION_IMPORT events from the original run may have been cleared (TRUNCATE),
      // so this ADOPTION_ALREADY_COMPLETE event serves as the canonical audit record
      // for every subsequent restart.
      // Write ADOPTION_ALREADY_COMPLETE on every restart where adoption was
      // previously completed. This is the canonical audit record when
      // ADOPTION_IMPORT events were wiped (TRUNCATE). Not silent on failure.
      let adoptionAuditWritten = false;
      try {
        const { data: currentPositions, error: posErr } = await supabase.from('positions')
          .select('asset, strategy_tag, state, origin, managed, qty_open, avg_cost_krw, operator_classified_at')
          .in('state', ['open', 'adopted', 'partial']);

        if (posErr) {
          console.warn('[adoption] ADOPTION_ALREADY_COMPLETE: failed to fetch positions:', posErr.message);
        } else {
          const adoptedPositions = (currentPositions || []).filter((p) => p.origin === 'adopted_at_startup');
          const protectedCount   = adoptedPositions.filter((p) => p.strategy_tag === 'unassigned').length;
          const classifiedCount  = adoptedPositions.filter((p) => p.strategy_tag !== 'unassigned').length;

          const { error: insertErr } = await supabase.from('bot_events').insert({
            event_type:   'ADOPTION_ALREADY_COMPLETE',
            severity:     'info',
            subsystem:    'portfolio_adopter',
            message:      `Adoption previously completed (run ${existing.id}) — ${adoptedPositions.length} adopted positions, ${protectedCount} unassigned, ${classifiedCount} classified`,
            context_json: {
              original_run_id:           existing.id,
              original_run_at:           existing.run_at,
              original_adopted_count:    existing.adopted_count,
              original_unsupported_count: existing.unsupported_count,
              current_positions: (currentPositions || []).map((p) => ({
                asset: p.asset, strategy_tag: p.strategy_tag, state: p.state,
                origin: p.origin, managed: p.managed, qty_open: Number(p.qty_open),
                avg_cost_krw: p.avg_cost_krw, operator_classified_at: p.operator_classified_at,
              })),
              protected_unassigned: protectedCount,
              classified_core:      classifiedCount,
            },
            mode,
          });

          if (insertErr) {
            console.warn('[adoption] ADOPTION_ALREADY_COMPLETE: insert failed:', insertErr.message);
          } else {
            adoptionAuditWritten = true;
            console.log(`[adoption] ADOPTION_ALREADY_COMPLETE written — ${adoptedPositions.length} positions documented`);
          }
        }
      } catch (err) {
        console.warn('[adoption] ADOPTION_ALREADY_COMPLETE: unexpected error:', err.message);
      }
  ...
  try {
    ...
    for (const acc of accounts) {
      ...
      console.log(`[adoption] ADOPTED: ${symbol} qty=${qty} avg=₩${Math.round(avgCostKrw ?? 0).toLocaleString()} mark=₩${Math.round(markPrice ?? 0).toLocaleString()} tag=tactical pos=${positionId}`);

      try { await supabase.from('bot_events').insert({
        event_type:   'ADOPTION_IMPORT',
        severity:     'info',
        subsystem:    'portfolio_adopter',
        message:      `Adopted ${symbol}: qty=${qty} mark=₩${Math.round(markPrice ?? 0).toLocaleString()} value=₩${Math.round(estimatedValue ?? 0).toLocaleString()} tag=tactical`,
        context_json: entry,
        mode,
      }); } catch (_) {}
    }
