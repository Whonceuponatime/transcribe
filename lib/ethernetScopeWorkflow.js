...
// Target-projected: skip pages with these in sheetTitle (noise/watermark/cover)
const NOISE_PAGE_PATTERNS = [
  /CONFIDENTIAL\s+UNAUTHORIZED\s+USE/i,
  /UNAUTHORIZED\s+USE/i,
  /^\s*COVER\s*$/i,
  /^\s*INDEX\s*$/i,
  /^\s*LEGEND\s*$/i,
  /^\s*ABBREVIATION/i,
  /LIST\s+OF\s+DRAWINGS/i,
  /REVISION\s*HISTORY/i
];
const NO_PAGE_CODE_SKIP = true; // skip pages with no valid page code when in target-projected mode

function isNoisePage(sheetTitle) {
  if (!sheetTitle || !String(sheetTitle).trim()) return true;
  const t = String(sheetTitle).trim();
  for (const re of NOISE_PAGE_PATTERNS) {
    if (re.test(t)) return true;
  }
  if (/^(INDEX|LEGEND|ABBREVIATION|COVER|CONTENTS)\s*$/i.test(t)) return true;
  return false;
}

// Reject cable IDs that look like dates or short page codes (not shipyard cable IDs)
const BAD_CABLE_ID_DATE = /^\d{4}-\d{2}-\d{2}$/;  // 2026-01-20, 2025-11-11
const BAD_CABLE_ID_SHORT = /^\d{2}-\d{3}$/;       // 32-002 (page-code style without letter)
function isBadCableId(cableId) {
  if (!cableId || !String(cableId).trim()) return true;
  const n = String(cableId).trim();
  if (BAD_CABLE_ID_DATE.test(n)) return true;
  if (BAD_CABLE_ID_SHORT.test(n)) return true;
  return false;
}
 ...
 * Optional 5th column: zone (e.g. Control, Nav & Comm, Untrusted, Cargo)
 */
function parseScopeCsv(csvText) {
  ...
  const hasHeader = /system|name|vendor|tag|code|zone/.test(header);
  ...
  for (let i = hasHeader ? 1 : 0; i < lines.length; i++) {
    ...
    rows.push({
      ...
      zone: (parts[4] || '').trim() || undefined
    });
  }
  return rows;
}

/** Infer zone from system name when not provided. Used for Zone & Conduit diagram. */
function inferZoneFromName(normName) {
  if (!normName) return 'Control';
  const upper = String(normName).toUpperCase();
  if (/\b(FIREWALL|EXTERNAL|UNTRUSTED|DMZ)\b/.test(upper)) return 'Untrusted';
  if (/\b(VSAT|CCTV|VDR|RADAR|RADIO|AIS|ECDIS|NAV|TELEPHONE|PA\b|COMM)\b/.test(upper)) return 'Nav & Comm';
  if (/\b(CARGO|TANK)\b/.test(upper)) return 'Cargo';
  return 'Control';
}
...
function buildScopeModel(scopeRows) {
  ...
  for (const row of scopeRows || []) {
    ...
    const zone = (row.zone && String(row.zone).trim()) || inferZoneFromName(baseName);
...
function buildSystemDictionary(scopeModel) {
  ...
  return scopeModel.map((row) => {
    ...
    const zone = (row.zone && String(row.zone).trim()) || inferZoneFromName(name);
    return {
      canonicalName,
...
/**
 * Resolve an endpoint label to a target system ID or UNMAPPED.
 * Uses manualAliases first, then fuzzy match against targets (systemId, aliases, strongKeywords).
 * Output is always one of the target systemIds or UNMAPPED.
 */
function resolveEndpointToTarget(label, targets, manualAliases) {
  if (!label || !String(label).trim()) return UNMAPPED;
  const upper = String(label).trim().toUpperCase();
  if (upper === 'UNKNOWN' || upper === 'PANEL' || upper === 'CONSOLE' || upper === 'UPS') return UNMAPPED;

  const aliasMap = manualAliases || {};
  for (const [key, value] of Object.entries(aliasMap)) {
    if (key.toUpperCase() === upper || upper.includes(key.toUpperCase())) return value;
  }

  for (const t of targets || []) {
    const sid = (t.systemId || t.systemName || '').toUpperCase();
    if (!sid) continue;
    if (upper === sid || upper.includes(sid)) return t.systemId || t.systemName;
    const aliases = t.aliases || [];
    for (const a of aliases) {
      const au = String(a).toUpperCase();
      if (au.length >= 2 && (upper === au || upper.includes(au) || au.includes(upper))) return t.systemId || t.systemName;
    }
    const keywords = t.strongKeywords || [];
    for (const k of keywords) {
      const ku = String(k).toUpperCase();
      if (ku.length >= 2 && upper.includes(ku)) return t.systemId || t.systemName;
    }
  }
  return UNMAPPED;
}
...
/**
 * Build target-projected topology: only targets as nodes, only edges where BOTH endpoints resolve to a target.
 * Returns edges_system, edges_detail, excluded_edges, target_coverage, review (unmapped prioritized).
 */
function buildTargetProjectedTopology(edges, review, pageSystemMap, targets, dictionary, manualAliases, options) {
  const targetIds = new Set((targets || []).map((t) => t.systemId || t.systemName).filter(Boolean));
  const pageToMeta = new Map();
  (pageSystemMap || []).forEach((m) => {
    const key = `${m.fileName}:${m.page}`;
    pageToMeta.set(key, m);
  });
  ...
  targetIds.forEach((id) => targetStats.set(id, { pagesMapped: 0, ethernetOccurrences: 0, edgesFound: 0 }));

  (pageSystemMap || []).forEach((m) => {
    if (m.systemId && targetIds.has(m.systemId) && !m.skipped) {
      const s = targetStats.get(m.systemId);
      if (s) s.pagesMapped += 1;
    }
  });

  function getPageMeta(fileName, page) {
    return pageToMeta.get(`${fileName}:${page}`) || null;
  }

  function isPageSkipped(fileName, page) {
    const m = getPageMeta(fileName, page);
    if (!m) return false;
    if (m.skipped) return true;
    if (NO_PAGE_CODE_SKIP && !m.pageCode && !extractPageCodeFromText(m.pageTitle)) return true;
    return false;
  }
  ...
  (edges || []).forEach((e) => {
    ...
    const fromRef = pageRefs[0] || sheetRefs[0];
    const toRef = pageRefs[1] || sheetRefs[1];
    const fromLabel = e.from && typeof e.from === 'object' ? (e.from.labelRaw || e.from.labelNormalized || '') : (e.from || '');
    const toLabel = e.to && typeof e.to === 'object' ? (e.to.labelRaw || e.to.labelNormalized || '') : (e.to || '');

    const fromClass = classifyEndpointBoundary(fromLabel, '');
    const toClass = classifyEndpointBoundary(toLabel, '');
    const fromBoundary = fromClass.class === 'boundary_device';
    const toBoundary = toClass.class === 'boundary_device';

    const systemA = resolveEndpointToTarget(fromLabel, targets, manualAliases);
    const systemB = resolveEndpointToTarget(toLabel, targets, manualAliases);

    edges_detail.push({
      ...e,
      fromResolvedTarget: systemA,
      toResolvedTarget: systemB,
      fromBoundary,
      toBoundary,
      fromClassification: fromClass,
      toClassification: toClass
    ...
    if (systemA === UNMAPPED || systemB === UNMAPPED) rejectReason = 'unmapped_endpoint';
    else if (!fromBoundary || !toBoundary) rejectReason = 'internal_only';
    else if (!isEthernetEdge(e)) rejectReason = 'non_ethernet';
    else if (isBadCableId(e.cableIdNormalized || e.cableIdRaw)) rejectReason = 'bad_cable_id';
    else if (fromLabel.toUpperCase() === 'UNKNOWN' || toLabel.toUpperCase() === 'UNKNOWN') rejectReason = 'unknown_endpoint';

    const fromPageSkipped = fromRef && isPageSkipped(fromRef.fileName, fromRef.page);
    const toPageSkipped = toRef && isPageSkipped(toRef.fileName, toRef.page);
    const sheetTitleFrom = (fromRef && getPageMeta(fromRef.fileName, fromRef.page)?.pageTitle) || '';
    const sheetTitleTo = (toRef && getPageMeta(toRef.fileName, toRef.page)?.pageTitle) || '';
    if (isNoisePage(sheetTitleFrom) || isNoisePage(sheetTitleTo)) rejectReason = rejectReason || 'noise_page';

    if (rejectReason) {
      excluded_edges.push({
        cableId: e.cableIdNormalized || e.cableIdRaw,
        fromLabel,
        toLabel,
        fromResolved: systemA,
        toResolved: systemB,
        reason: rejectReason,
        pageRefs: e.pageRefs || []
      });
    ...
    if (systemA === systemB) return;
    if (!targetIds.has(systemA) || !targetIds.has(systemB)) return;
    ...
    const sa = targetStats.get(systemA);
    const sb = targetStats.get(systemB);
    if (sa) sa.ethernetOccurrences += 1;
    if (sb) sb.ethernetOccurrences += 1;
  });

  for (const agg of pairToEdge.values()) {
    const sa = targetStats.get(agg.fromSystemId);
    const sb = targetStats.get(agg.toSystemId);
    if (sa) sa.edgesFound += 1;
    if (sb) sb.edgesFound += 1;
  }
  ...
  for (const id of targetIds) {
    const s = targetStats.get(id) || { pagesMapped: 0, ethernetOccurrences: 0, edgesFound: 0 };
    let reason = null;
    if (s.edgesFound === 0 && s.pagesMapped === 0) reason = 'no_pages_mapped';
    else if (s.edgesFound === 0) reason = 'no_ethernet_edges';
    target_coverage.push({
      target: id,
      pagesMapped: s.pagesMapped,
      ethernetOccurrences: s.ethernetOccurrences,
      edgesFound: s.edgesFound,
      reason
    });
  }

  const unmappedLabels = new Map();
  excluded_edges.filter((x) => x.reason === 'unmapped_endpoint').forEach((x) => {
    if (x.fromResolved === UNMAPPED) {
      const lbl = (x.fromLabel || '').trim() || '—';
      unmappedLabels.set(lbl, (unmappedLabels.get(lbl) || 0) + 1);
    }
    if (x.toResolved === UNMAPPED) {
      const lbl = (x.toLabel || '').trim() || '—';
      unmappedLabels.set(lbl, (unmappedLabels.get(lbl) || 0) + 1);
    }
  });

  const reviewWithUnmapped = [...(review || [])];
  for (const ex of excluded_edges) {
    if (ex.reason === 'unmapped_endpoint') {
      reviewWithUnmapped.unshift({
        type: 'unmapped_endpoint',
        cableId: ex.cableId,
        fromLabel: ex.fromLabel,
        toLabel: ex.toLabel,
        fromResolved: ex.fromResolved,
        toResolved: ex.toResolved,
        reason: 'Map endpoint to a target or add alias',
        pageRefs: ex.pageRefs || []
      });
    }
  }
  ...
  return {
    ...
    excluded_edges,
    target_coverage,
    review: reviewWithUnmapped,
    unmappedLabels: Array.from(unmappedLabels.entries()).map(([label, count]) => ({ label, count })).sort((a, b) => b.count - a.count)
  };
}

/**
 * Assign conduit class to each system-level edge for Zone & Conduit diagram.
 * Rules: one end Untrusted → tcpip_untrusted; different zones → zone_traversal; else tcpip_internal.
 */
function assignConduitClass(edges_system, systems) {
  const zoneBySystem = new Map();
  (systems || []).forEach((s) => {
    const id = s.systemId || s.systemName;
    if (id) zoneBySystem.set(id, (s.zone || 'Control').trim());
  });

  return (edges_system || []).map((e) => {
    const zoneA = zoneBySystem.get(e.fromSystemId) || 'Control';
    const zoneB = zoneBySystem.get(e.toSystemId) || 'Control';
    const untrusted = (z) => String(z).toLowerCase() === 'untrusted';
    let conduitClass = 'tcpip_internal';
    if (untrusted(zoneA) || untrusted(zoneB)) conduitClass = 'tcpip_untrusted';
    else if (zoneA !== zoneB) conduitClass = 'zone_traversal';
    return { ...e, conduitClass };
  });
}
...
function runScopeFirstWorkflow(scopeList, extractionResult, pageMetas, options = {}) {
  ...
  const systems = dictionary.map((d) => {
    ...
    return {
      ...
      displayName: displayLabel || d.canonicalName,
  ...
  const manualAliases = options.manualAliases || {};
  const page_system_map = metas.map((m) => {
    const mapping = mapPageToSystem(m.pageTitle, m.pageCode, dictionary, mapOptions);
    const hasValidCode = !!(m.pageCode || extractPageCodeFromText(m.pageTitle));
    const noise = isNoisePage(m.pageTitle);
    const skipped = noise || (NO_PAGE_CODE_SKIP && !hasValidCode);
    let skipReason = null;
    if (noise) skipReason = 'noise_page';
    else if (NO_PAGE_CODE_SKIP && !hasValidCode) skipReason = 'no_valid_page_code';
    return {
      fileName: m.fileName,
      page: m.page,
  ...
  const useTargetProjected = systems.length > 0;
  let edges_system = [];
  let edges_detail = [];
  let review = reviewWithUnknown;
  let excluded_edges = [];
  let target_coverage = [];
  let unmappedLabels = [];

  if (useTargetProjected) {
    const projected = buildTargetProjectedTopology(
    ...
    edges_system = assignConduitClass(projected.edges_system || [], systems);
    edges_detail = projected.edges_detail;
    review = projected.review;
    excluded_edges = projected.excluded_edges || [];
    target_coverage = projected.target_coverage || [];
    unmappedLabels = projected.unmappedLabels || [];
  } else {
    ...
    edges_system = topo.edges_system;
    edges_detail = topo.edges_detail;
    review = topo.review;
  }
  ...
  const summary = {
    ...
    pagesMapped: page_system_map.filter((p) => p.systemId && p.systemId !== UNKNOWN_SYSTEM && !p.out_of_scope && !p.skipped).length,
    ...
    excluded_edges: excluded_edges.length,
    target_projected: useTargetProjected
  };
...
/**
 * Optional: AI resolver for unmapped endpoints. May only return a target systemId from the list or UNMAPPED.
 * Must return evidence tokens (snippets + page refs). Critic pass: if critic disagrees, keep unmapped.
 * Stub: returns UNMAPPED when openai not provided.
 */
async function aiResolveUnmappedEndpoint(label, pageRefs, targetSystemIds, openai, model) {
  if (!openai || !label) return { resolved: UNMAPPED, confidence: 0, evidence: [] };
  // Stub: in a full impl, call openai with label + context, strict instructions to only return one of targetSystemIds or UNMAPPED + evidence.
  return { resolved: UNMAPPED, confidence: 0, evidence: [] };
}
