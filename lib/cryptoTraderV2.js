...
async function executeCycleV2(supabase, opts = {}) {
  ...
  try {
    ...
    for (const coin of coins) {
      ...
      // Replaces the old fixed max_risk_per_signal_pct formula with a deployment
      // target that scales with portfolio size. Each entry gets an even slice of
      // the remaining room for this asset.
      const targetDeploymentPct   = cfg.target_deployment_pct ?? 30;
      const targetEntriesPerPos   = cfg.target_entries_per_position ?? 8;
      const maxPerAssetKrw        = portfolio.navKrw * (targetDeploymentPct / 100);
