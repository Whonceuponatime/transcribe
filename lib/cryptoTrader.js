  ...
  for (const coin of coins) {
    ...
    if (config.signal_sell_enabled && sig && !sig.error) {
      ...
      if (overCashed) {
        ...
        console.log(`    Signal sells: PAUSED — sold in last 6h (global cooldown)`);
      } else {
      ...
      console.log(`      RSI ob_strong   : RSI=${sig.rsi?.toFixed(1) ?? '—'} (need >85)${atProfit ? '' : ' [atProfit✗]'}`);
      console.log(`      RSI ob          : RSI=${sig.rsi?.toFixed(1) ?? '—'} (need >75)${atProfit ? '' : ' [atProfit✗]'}`);
    ...
    if (config.stop_loss_pct > 0 && gainPct != null && gainPct < -config.stop_loss_pct) {
      ...
      const { data: slData } = await supabase.from('crypto_bot_logs')
      ...
      const { data: lastBuyData } = await supabase.from('crypto_trade_log')
        .select('executed_at').eq('coin', coin).eq('side', 'buy')
        .order('executed_at', { ascending: false }).limit(1).single().catch(() => ({ data: null }));
