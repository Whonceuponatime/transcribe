...
// Balance-flicker stabilization: when Upbit transiently returns exchange=0
// against a non-zero DB qty (common immediately after a settled trade),
// wait and re-fetch once before declaring a freeze.
const FLICKER_RETRY_DELAY_MS = 3000;
...
const AUTO_ADOPT_ELIGIBLE = new Set(['BTC', 'ETH', 'XRP']);
...
const AUTO_ADOPT_DUST_MIN = { BTC: 0.000001, ETH: 0.00001, XRP: 1.0 };
...
async function checkBalanceMatch(supabase, accounts, supportedCoins) {
  ...
  for (const coin of supportedCoins) {
    let   exQty  = exchangeQty[coin] ?? 0;
    const intQty = dbQty[coin] ?? 0;

    // Skip if both are zero — nothing to compare
    if (exQty === 0 && intQty === 0) continue;

    // Balance-flicker stabilization.
    // Upbit transiently returns 0 for an asset immediately after a settled
    // trade. A single API reading is therefore not authoritative — before
    // freezing on exchange=0/db>0, wait and re-fetch once. If the retry
    // returns a real value, use that; otherwise proceed with the freeze.
    if (exQty === 0 && intQty > 0) {
      await sleep(FLICKER_RETRY_DELAY_MS);
      try {
        const retryAccounts = await upbit.getAccounts();
        const retryAcc      = retryAccounts.find((a) => a?.currency === coin);
        const retryQty      = Number(retryAcc?.balance ?? 0) + Number(retryAcc?.locked ?? 0);
        if (retryQty > 0) {
          console.warn(`[reconcile] ⚠️ ${coin} balance flicker: exchange briefly returned 0, retry=${retryQty} — continuing without freeze`);
          exQty              = retryQty;
          exchangeQty[coin]  = retryQty; // keep return map consistent
        } else {
          console.log(`[reconcile] ${coin} balance flicker retry: still 0 after ${FLICKER_RETRY_DELAY_MS}ms — proceeding with freeze`);
        }
      } catch (retryErr) {
        console.warn(`[reconcile] ${coin} balance flicker retry failed: ${retryErr?.message || retryErr} — proceeding with original reading`);
      }
    }
...
async function attemptAutoAdopt(supabase, accounts, supportedCoins) {
  ...
  if (adoptedAssets.length > 0) {
    try {
      ...
      console.error('[reconcile/auto-adopt] adoption_runs check/insert failed:', err.message);
    }
  }
...
async function runReconciliation(supabase, supportedCoins = ['BTC', 'ETH', 'SOL'], trigger = 'startup', force = false) {
  ...
  // ── Guarded auto-adopt ─────────────────────────────────────────────────────
  // Trigger condition: any eligible coin where exchange has holdings above dust
  // but DB has zero active managed positions — regardless of adoption_run history.
  //
  // The previous guard (checking adoption_run completion) was wrong: when a prior
  // adoption_run exists and runAdoption skips as alreadyDone, checkAdoptionComplete
  // passes, positions still don't exist, and auto-adopt was never called before the
  // balance_mismatch freeze fired. The correct trigger is the actual mismatch state.
